...
class ElasticForm extends CComponent
{
     ...
     */
    private $renderingTemplate = false;
    /**
     ...
     *
     * @return \sweelix\yii1\behaviors\ElasticModel
     * @since  1.0.0
     */
    ...
    {
        $templateId = $this->parentModel->templateId;
        $this->renderingTemplate = $this->parentModel->getRenderingTemplate($templateId);
        if ($this->renderingTemplate === false) {
            foreach ($this->parentModel->getTemplateConfig() as $elementName => $elementConfig) {
    ...
    public function render($return = true)
    {
        ...
        if ($this->renderingTemplate === false) {
            ...
            $result = $controller->renderFile($this->renderingTemplate, array('model' => $this->parentModel), $return);
            if ($return === true) {
                return $result;
            } else {
                return true;
